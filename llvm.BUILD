...
# System LLVM is installed via: clang18-devel, llvm18-devel, lld18, libcxx-devel
# Similar to how openssl.BUILD wraps system OpenSSL
#
# Repository path is "/usr", so paths below are relative to /usr:
# - bin/clang-18 resolves to /usr/bin/clang-18
# - lib64/clang/18/include/ resolves to /usr/lib64/clang/18/include/
#
# Note: The Dockerfile creates symlinks like /usr/bin/clang -> /usr/bin/clang-18

package(default_visibility = ["//visibility:public"])

# Clang compiler builtin headers (stddef.h, limits.h, etc.)
# Located at /usr/lib/clang/18/include/ on Fedora 43 (lib, not lib64!)
# Note: /lib is a symlink to usr/lib, so paths may appear as either /lib/clang or /usr/lib/clang
filegroup(
    name = "clang_builtin_headers",
    srcs = glob([
        "lib/clang/18/include/**/*.h",
...
# Note: We use libc++ headers from LLVM 18.1.8 distribution (symlinked at /usr/include/c++/v1)
# instead of Fedora 43's libc++ which requires Clang 19+
filegroup(
    name = "include",
    srcs = glob([
        "lib64/llvm18/include/**",
        "lib/clang/18/include/**",     # Compiler builtin headers (stddef.h, limits.h, etc.)
        "include/c++/v1/**",            # libc++ headers compatible with Clang 18 (symlinked)
    ]),
)
...
filegroup(
    ...
    srcs = [
        "bin/clang",      # Symlink created by Dockerfile: /usr/bin/clang -> /usr/bin/clang-18
